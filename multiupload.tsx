...
// Props do componente final, conforme definido
interface CorporateListProps {
  data: ListItem[];
...
const Badge: React.FC<BadgeProps> = ({ status, size = 'md' }) => {
  ...
  const config = getStatusConfig(status);
  const sizeClasses = { sm: 'px-2 py-1 text-xs', md: 'px-3 py-1 text-sm', lg: 'px-4 py-2 text-base' };
...
// CorporateList Component (Versão Final)
const CorporateList: React.FC<CorporateListProps> = ({ data, onItemsAdded, isLoading }) => {
  ...
  return (
    <div
      className={`relative bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden group ${isDraggingOver ? 'border-blue-500 border-dashed' : ''} ${isLoading ? 'opacity-60 pointer-events-none' : ''}`}
      onDragOver={handleDragOver}
    ...
    >
      ...
      {isLoading && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-white bg-opacity-80">
          ...
          <p className="text-lg font-semibold text-blue-800">Enviando arquivos...</p>
        </div>
      )}
      ...
      <div className="px-6 py-4 border-b border-gray-200" style={{ backgroundColor: '#FAFAFA' }}>
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-8 md:col-span-5"><h3 className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Nome</h3></div>
          <div className="hidden md:block md:col-span-5"><h3 className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Descrição</h3></div>
          <div className="col-span-4 md:col-span-2 text-center"><h3 className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Status</h3></div>
        </div>
      </div>
      ...
      <div className="divide-y divide-gray-200">
        {data.length === 0 ? (
          <div className="px-6 py-12 text-center cursor-pointer hover:bg-gray-50" onClick={triggerFileSelect}>
            <i className="fas fa-cloud-upload-alt text-4xl mb-4 text-gray-500"></i>
            <p className="text-lg font-medium mb-2 text-gray-800">Arraste e solte seus arquivos aqui</p>
            <p className="text-sm text-gray-600">ou <span className="font-semibold text-blue-600">clique para selecionar</span></p>
          </div>
        ) : (
          data.map((item, index) => (
            <div
            ...
            >
              <div className="col-span-8 md:col-span-5 flex items-center min-w-0">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs mt-1 truncate md:hidden text-gray-500">{item.description}</p>
                </div>
              </div>
              <div className="hidden md:block md:col-span-5 flex items-center min-w-0"><p className="text-sm truncate text-gray-500">{item.description}</p></div>
              <div className="col-span-4 md:col-span-2 flex items-center justify-center">
                {item.status && <Badge status={item.status} size="sm" />}
              </div>
            </div>
...
function App() {
  ...
  const handleItemsAdded = (files: File[]): void => {
    // 1. Cria os itens visuais com status 'pending' para feedback imediato
    const newPendingItems: ListItem[] = files.map(file => ({
      name: file.name,
    ...
    setIsLoading(true);
    console.log('Iniciando upload simulado para:', files.map(f => f.name));

    // 3. Simula um processo de upload (ex: 2 segundos)
    setTimeout(() => {
      console.log('Upload simulado concluído.');
      
      // 4. Atualiza os itens de 'pending' para 'succeed'
      setListItems(currentItems => 
        currentItems.map(item => 
          newPendingItems.some(pendingItem => pendingItem.name === item.name)
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="mb-8 text-gray-600">
          Arraste arquivos para a lista abaixo ou clique para selecionar. O estado é controlado pelo componente pai.
        </p>
